...
ToneMapping::ToneMapping(HINSTANCE hInstance) : GameApp(hInstance)
{
...
void ToneMapping::SanitizeCamera(Vector3& eye, Vector3& target, Vector3& up)
{
	// eye == target 방지
...
bool ToneMapping::Initialize(UINT Width, UINT Height)
{
...
void ToneMapping::Update()
{
	__super::Update();
	SetMatrix();
	CanUseHDR = CheckUseHDR(m_pSwapChain3);
}
...
bool ToneMapping::InitD3D()
{
	...
	m_Width = GameApp::m_ClientWidth;
	m_height = GameApp::m_ClientHeight;
	aspect = m_Width / m_height;

	return true;
}

void ToneMapping::UninitD3D()
{
...
LRESULT ToneMapping::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
...
void ToneMapping::DebugMatrix4x4(const char* label, const Matrix& matrix)
{
...
void ToneMapping::SetDebugLightFrustum()
{
...
void ToneMapping::RenderDebugLightFrustum()
{
...
void ToneMapping::SetPBRValue(std::shared_ptr<ModelLoader> mesh)
{
...
void ToneMapping::SetIBLValue(std::shared_ptr<ModelLoader> mesh)
{
	if (useIBL)
	{
...
void ToneMapping::RenderToneMappPass()
{
...
bool ToneMapping::CheckUseHDR(IDXGISwapChain* swapChain)
{
