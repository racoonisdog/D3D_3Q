...
private:
    ...
    void SetTextureType(ComPtr<ID3D11DeviceContext>& dev, int _index)
    {
        ...
        else if (typeN == "METALLIC")
        {
            dev->PSSetShaderResources(4, 1, textures_[_index].texture.GetAddressOf());
        }
        else if (typeN == "ROUGHNESS")
        {
            dev->PSSetShaderResources(5, 1, textures_[_index].texture.GetAddressOf());
        }
    }
};
