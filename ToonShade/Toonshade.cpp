...
Toonshade::Toonshade(HINSTANCE hInstance) : GameApp(hInstance)
{
...
void Toonshade::SetMatrix()
{
	XMMATRIX S = XMMatrixScaling(P_SkinningTestScale.x, P_SkinningTestScale.y, P_SkinningTestScale.z);
	XMMATRIX R = XMMatrixRotationRollPitchYaw(P_SkinningTestrotation.x, P_SkinningTestrotation.y, P_SkinningTestrotation.z);
	XMMATRIX T = XMMatrixTranslation(P_SkinningTestposition.x, P_SkinningTestposition.y, P_SkinningTestposition.z);
...
void Toonshade::SanitizeCamera(Vector3& eye, Vector3& target, Vector3& up)
{
	// eye == target 방지
...
bool Toonshade::Initialize(UINT Width, UINT Height)
{
...
void Toonshade::Update()
{
	__super::Update();
	SetMatrix();
	if(checkChange)	SetTwotoThree();
}

void Toonshade::Render()
{
	...
	//리소스
	m_pDeviceContext->PSSetShaderResources(4, 1, m_pToonTexture.GetAddressOf());
	m_pDeviceContext->PSSetShaderResources(5, 1, m_pQuantizedTexture.GetAddressOf());
	...
	//아웃라인변수 false인 1 설정
	OutLineCS.flag = 1;
	m_pDeviceContext->UpdateSubresource(m_pOutLineConstantBuffer.Get(), 0, nullptr, &OutLineCS, 0, 0);
	m_pDeviceContext->VSSetConstantBuffers(3, 1, m_pOutLineConstantBuffer.GetAddressOf());
	m_pDeviceContext->PSSetConstantBuffers(3, 1, m_pOutLineConstantBuffer.GetAddressOf());
	...
	//상수버퍼 flag int로 던져주는용
	OutLineCS.flag = 2;
	OutLineCS.OutlineThickness = linevalue;
	m_pDeviceContext->UpdateSubresource(m_pOutLineConstantBuffer.Get(), 0, nullptr, &OutLineCS, 0, 0);
	m_pDeviceContext->VSSetConstantBuffers(3, 1, m_pOutLineConstantBuffer.GetAddressOf());
	m_pDeviceContext->PSSetConstantBuffers(3, 1, m_pOutLineConstantBuffer.GetAddressOf());
	m_pDeviceContext->RSSetState(m_OutLineRasterizer.Get());
	SkinningTest.get()->DrawSkeletal(m_pDeviceContext, m_pMaterialBuffer, m_pBlendON, m_pBlendOFF);
...
bool Toonshade::InitD3D()
{
	...
	rd.FrontCounterClockwise = FALSE;
	rd.DepthClipEnable = TRUE;
	m_pDevice->CreateRasterizerState(&rd, m_OutLineRasterizer.GetAddressOf());
	///
...
bool Toonshade::InitScene()
{
	...
	//변수 설정용 상수버퍼
	cbDesc.ByteWidth = (sizeof(OutLine) + 15) & ~15;;
	HR_T(hr = m_pDevice->CreateBuffer(&cbDesc, nullptr, m_pOutLineConstantBuffer.GetAddressOf()));


	HR_T(CreateTextureFromFile(m_pDevice.Get(), L"resource\\SpecularRampTexture1.png", m_pToonTexture.GetAddressOf()));
	HR_T(CreateTextureFromFile(m_pDevice.Get(), L"resource\\SpecularRampTexture2.png", m_pQuantizedTexture.GetAddressOf()));
...
void Toonshade::RenderGUI()
{
...

	{
		...
		ImGui::PushID(6);
		ImGui::Text("LineOption");
		ImGui::DragFloat("thickness", &linevalue, 0.001f, 0.0f, 20.0f - eps_local);
		ImGui::ColorEdit4("Color", L_LineColor);
		ImGui::Checkbox("Toon two tone", &Toontwo);
		{ checkChange = true; }
		ImGui::Checkbox("Quantized two tone", &Quantizedtwo);
		{ checkChange = true; }
		if (ImGui::Button("Reset##Line")) {
			linevalue = 0.01f;
			P_SkinningTestrotation = XMFLOAT3(0.0f, 0.0f, 0.0f);
			L_LineColor[0] = 1.0f; L_LineColor[1] = 1.0f; L_LineColor[2] = 1.0f; L_LineColor[3] = 1.0f;
		}
		ImGui::DragFloat("shininess", &constandices.shininess, 1.0f, 1000.0f, 20000.0f);
		if (ImGui::Button("Reset##shininess")) {
			constandices.shininess = 2000.0f;
		}
		OutLineCS.LineColor = { L_LineColor[0], L_LineColor[1], L_LineColor[2], L_LineColor[3] };
...
LRESULT Toonshade::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
...
void Toonshade::SetRenderSort()
{

}

void Toonshade::SetTwotoThree()
{
	if (Toontwo)
	{
		HR_T(CreateTextureFromFile(m_pDevice.Get(), L"resource\\SpecularRampTexture3.png", m_pToonTexture.GetAddressOf()));
	}
	else
	{
		HR_T(CreateTextureFromFile(m_pDevice.Get(), L"resource\\SpecularRampTexture1.png", m_pToonTexture.GetAddressOf()));
	...
	if (Quantizedtwo)
	{
		HR_T(CreateTextureFromFile(m_pDevice.Get(), L"resource\\SpecularRampTexture4.png", m_pQuantizedTexture.GetAddressOf()));
	}
	else
	{
		HR_T(CreateTextureFromFile(m_pDevice.Get(), L"resource\\SpecularRampTexture2.png", m_pQuantizedTexture.GetAddressOf()));
