...
struct Constant
{
	...
	DirectX::XMFLOAT4 lightspecular;		//정반사
...
class Toonshade : public GameApp
{
public:
	Toonshade(HINSTANCE hInstance);
	~Toonshade();
	...
	ComPtr<ID3D11Buffer> m_pOutLineConstantBuffer = nullptr;			// 아웃라인변수버퍼
	...
	float linevalue = 0.01;
	float L_LineColor[4] = { 1.0f, 1.0f, 1.0f, 1.0f };

	ComPtr<ID3D11RasterizerState> m_OutLineRasterizer = nullptr; //아웃라인용
	ComPtr<ID3D11ShaderResourceView> m_pToonTexture = nullptr;	// 툰셰이딩용 텍스쳐
	ComPtr<ID3D11ShaderResourceView> m_pQuantizedTexture = nullptr;	// 툰셰이딩용 텍스쳐

	bool Toontwo = false;
	bool Quantizedtwo = false;
