...
SkyBox::SkyBox(HINSTANCE hInstance) : GameApp(hInstance)
{
...
void SkyBox::SetMatrix()
{
	...
	//카메라 월드 계산
	m_CWorld = Matrix::CreateFromYawPitchRoll(m_Rotation) * Matrix::CreateTranslation(m_Position);
...
void SkyBox::SanitizeCamera(Vector3& eye, Vector3& target, Vector3& up)
{
	 // eye == target 방지
...
bool SkyBox::Initialize(UINT Width, UINT Height)
{
...
bool SkyBox::InitScene()
{
	...
	HR_T(CreateDDSTextureFromFile(m_pDevice, L"../resource/single/H.dds", nullptr, &m_pTextureRV));
...
void SkyBox::RenderGUI()
{
...

	{
		...
		if (ImGui::IsKeyDown(ImGuiKey_W)) CamPosition[2] += dv;
		if (ImGui::IsKeyDown(ImGuiKey_S)) CamPosition[2] -= dv;
		if (ImGui::IsKeyDown(ImGuiKey_A)) CamPosition[0] -= dv;
		if (ImGui::IsKeyDown(ImGuiKey_D)) CamPosition[0] += dv;
		if (ImGui::IsKeyDown(ImGuiKey_E)) CamPosition[1] += dv;
		if (ImGui::IsKeyDown(ImGuiKey_Q)) CamPosition[1] -= dv;

		// 적용
		m_Position = Vector3{ CamPosition[0], CamPosition[1],CamPosition[2] };
		m_Rotation = Vector3{ CamRotation[0] , CamRotation[1], CamRotation[2]};
...
LRESULT SkyBox::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
