...
constexpr const char* MyEnumNames[] =
{
	"SkyBox",
	"InDoor",
	"OutDoor",
...
class DeferredRendering : public GameApp
{
public:
	DeferredRendering(HINSTANCE hInstance);
	~DeferredRendering();
	...
	std::shared_ptr<ModelLoader> Sphere = nullptr;
	...
	//world 관련 변수 //
	XMFLOAT3 P_Sphereposition{ 2.0f, 1.0f, 0.0f };
	XMFLOAT3 P_Sphererotation{ 0.0f, 0.0f, 0.0f };
	XMFLOAT3 P_SphereScale{ 0.01f, 0.01f, 0.01f };
	...
	//IBL
	ComPtr<ID3D11ShaderResourceView> Sky_IBLDiffuse;
	ComPtr<ID3D11ShaderResourceView> Sky_IBLSpecular;
	ComPtr<ID3D11ShaderResourceView> Sky_IBLBrdf;


	ComPtr<ID3D11ShaderResourceView> In_IBLDiffuse;
	ComPtr<ID3D11ShaderResourceView> In_IBLSpecular;
	ComPtr<ID3D11ShaderResourceView> In_IBLBrdf;

	ComPtr<ID3D11ShaderResourceView> Out_IBLDiffuse;
	ComPtr<ID3D11ShaderResourceView> Out_IBLSpecular;
	ComPtr<ID3D11ShaderResourceView> Out_IBLBrdf;


	ComPtr<ID3D11ShaderResourceView> S_OutdoorSRV;   // OutdoorCube
	ComPtr<ID3D11ShaderResourceView> S_IndoorSRV;   // IndoorCube


	ComPtr<ID3D11SamplerState> m_pSamplerClamp{}; // IBL specular 샘플러

	bool useIBL = true;
	float AmbientOcclusion = 1.0f;

	void SetIBLValue(std::shared_ptr<ModelLoader> mesh);

	IBLType IBLvalue = IBLType::SkyBox;

	void SetSkyBoxMap();
	void SetIBLMap();

	//HDR용
	ComPtr<ID3D11Texture2D> m_SceneHDR_RT{};
	ComPtr<ID3D11ShaderResourceView> m_SceneHDR_SRV{};
	ComPtr<ID3D11RenderTargetView> m_SceneHDR_RTV{};

	ComPtr<ID3D11VertexShader> m_pToneMapFullScreenVS{};		// 정점 셰이더.
	ComPtr<ID3D11PixelShader> m_pToneMapLDRPS{};		// 픽셀 셰이더
	ComPtr<ID3D11PixelShader> m_pToneMapHDRPS{};		// 픽셀 셰이더
	ComPtr<ID3D11PixelShader> m_pDeferredPS{};		// 픽셀 셰이더
	ComPtr<ID3D11PixelShader> m_pDeferredLightPS{};		// 픽셀 셰이더

	ComPtr<ID3D11Buffer> m_ToneMapConstantBuffer{};			// 상수 버퍼.

	ToneMapValue toneValue{};

	float m_Exposure = 0.1f;
	float m_Gamma = 2.2f;
	float m_LightIntensity = 1.0f;
	float m_MaxHDRNits = 500.0f;

	void RenderToneMappPass();
	...
	IDXGISwapChain3* m_pSwapChain3 = nullptr;

	bool setLDRValue = true;
	...
	//DeferredRendering
	ComPtr<ID3D11Texture2D> g_BufferBaseColorTex;
	ComPtr<ID3D11RenderTargetView> g_BufferBaseColorRTV;
	ComPtr<ID3D11ShaderResourceView> g_BufferBaseColorSRV;

	ComPtr<ID3D11Texture2D> g_BufferNormalTex;
	ComPtr<ID3D11RenderTargetView> g_BufferNormalRTV;
	ComPtr<ID3D11ShaderResourceView> g_BufferNormalSRV;

	ComPtr<ID3D11Texture2D> g_BufferMetallicTex;
	ComPtr<ID3D11RenderTargetView> g_BufferMetallicRTV;
	ComPtr<ID3D11ShaderResourceView> g_BufferMetallicSRV;

	ComPtr<ID3D11Texture2D> g_BufferRoughnessTex;
	ComPtr<ID3D11RenderTargetView> g_BufferRoughnessRTV;
	ComPtr<ID3D11ShaderResourceView> g_BufferRoughnessSRV;

	ComPtr<ID3D11Texture2D> g_BufferPositionTex;
	ComPtr<ID3D11RenderTargetView> g_BufferPositionRTV;
	ComPtr<ID3D11ShaderResourceView> g_BufferPositionSRV;

	ComPtr<ID3D11Texture2D> g_BufferEmissionTex;
	ComPtr<ID3D11RenderTargetView> g_BufferEmissionRTV;
	ComPtr<ID3D11ShaderResourceView> g_BufferEmissionSRV;

	bool InitDeferredRender();
